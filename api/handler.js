...
async function whoopCallback(req, res, host, url) {
  try {
    ...
    if (!code) {
      const q = (url.search || "").replace(/^\?/, "") || "(rien recu)";
      res.statusCode = 302; res.setHeader("Location", "https://" + host + "/?error=whoop&detail=" + encodeURIComponent("WHOOP a renvoye: " + q)); res.end(); return;
    }
    const redirect = "https://" + host + "/api/whoop-callback";
    ...
    if (!tok.refresh_token) {
      const d = encodeURIComponent(((tok.error || "") + " " + (tok.error_description || JSON.stringify(tok))).slice(0, 160));
      res.statusCode = 302; res.setHeader("Location", "https://" + host + "/?error=whoop&detail=" + d); res.end(); return;
    }
    res.statusCode = 302;
    res.setHeader("Location", "https://" + host + "/#whoop=" + encodeURIComponent(tok.refresh_token));
    res.end();
  } catch (e) {
    res.statusCode = 302; res.setHeader("Location", "https://" + host + "/?error=whoop&detail=" + encodeURIComponent(String((e && e.message) || e))); res.end();
  }
}
...
async function withingsCallback(req, res, host, url) {
  try {
    ...
    if (!rt) {
      const d = encodeURIComponent((("status " + j.status + " ") + (j.error || JSON.stringify(j))).slice(0, 160));
      res.statusCode = 302; res.setHeader("Location", "https://" + host + "/?error=withings&detail=" + d); res.end(); return;
    }
    res.statusCode = 302;
    res.setHeader("Location", "https://" + host + "/#withings=" + encodeURIComponent(rt));
    res.end();
  } catch (e) {
    res.statusCode = 302; res.setHeader("Location", "https://" + host + "/?error=withings&detail=" + encodeURIComponent(String((e && e.message) || e))); res.end();
  }
}
...
module.exports = async function (req, res) {
  ...
  const path = url.searchParams.get("action") || url.pathname.replace(/^\/api\//, "").replace(/\/+$/, "");
